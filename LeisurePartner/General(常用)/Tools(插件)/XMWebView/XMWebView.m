...
- (instancetype)initWithFrame:(CGRect)frame viewType:(WebViewType)viewType {
    ...
    if (self) {
        ...
        if(self.wkWebView) {
            ...
            if (self.isiPhoneX) {
                // iPhoneX
                            self.progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, 88, frame.size.width, 2)];
            } else {
                // other
            ...
            //self.progressView.tintColor = XMRGBCOLOR(254, 206, 69);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if (object && [object isEqual:self.wkWebView] && [keyPath isEqualToString:@"estimatedProgress"]) { // 进度条
        ...
        NSLog(@"WebView加载进度值：%f", newprogress);
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if(alertView == self.externalAppPermissionAlertView) {
        if(buttonIndex != alertView.cancelButtonIndex) {
            ...
            [application openURL:self.URLToLaunchWithPermission options:@{} completionHandler:nil];
        }
        self.URLToLaunchWithPermission = nil;
    }
}

- (void)loadJsByJson:(NSString *)ScriptString{
    if (self.wkWebView) {
        [self.wkWebView evaluateJavaScript:ScriptString completionHandler:^(id _Nullable item, NSError * _Nullable error) {
            NSLog(@"wkWebView opering ScriptString...");
        }];
    } else if (self.uiWebView) {
        NSLog(@"uiWebView opering ScriptString...");
        [self.uiWebView stringByEvaluatingJavaScriptFromString:ScriptString];
    }
}
